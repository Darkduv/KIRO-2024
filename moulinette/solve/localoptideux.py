...
def solve(instance: Instance):
    ...
    job_ids = [job.id_ for job in instance.jobs]
    for schedule_nb in range(len(instance.jobs)):
        progress.advance(1)
        if random.random() <= 0.8:
            jobs_test = [(test_job(instance, Job.by_id(job_id))[0], job_id) for job_id in job_ids]
            _, job_id = min(jobs_test)
        else:
            job_id = random.choice(job_ids)
        # print("job_id choisi = ", job_id)
        _, scheduled_job = test_job(instance, Job.by_id(job_id), restore_time=False)
        # print("scheduled = ", *[(schedule, Task.by_id(schedule.task).processing_time) for schedule in scheduled_job], sep="\n    *  ")
