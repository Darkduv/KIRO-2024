...
from model.judge import reseq_2tones, score, reseq_2tones_inv
...
def simply_reseq_ass(instance:Instance, order:list[int])->Solution:
    post_paint = order
    
    two_tones = [v.id for v in instance.vehicles if v.type == VehicleType.TWOTONE]

    initial = reseq_2tones_inv(
        sigma=post_paint,
        two_tones=two_tones,
        delta=instance.parameters.two_tone_delta,
    )
    ...
    sol = Solution(
        ...
        assembly_entry=post_paint,
        assembly_exit=post_paint,
    )
...
def solve(instance: Instance)->Solution:
    ...
    lot_constraints = [c for c in constraints if isinstance(c, LotChangeConstraint) and c.shop=="paint"]
    window_constraints = [c for c in constraints if isinstance(c, RollingWindowConstraint) and c.shop=="paint"]
    batch_constraints = [c for c in constraints if isinstance(c, BatchSizeConstraint) and c.shop=="paint"]
